<?php
/**
 * This file is part of the Japanese Consumption Tax Extension For Magento2 project.
 *
 * Copyright (c) 2023 Adobe (or other copyright holders)
 *
 * For the full copyright and license information, please view the OSL-3.0
 * license that is bundled with this source code in the file LICENSE, or
 * at https://opensource.org/licenses/OSL-3.0
 */
namespace Magentoj\JapaneseConsumptionTax\Observer;

use \Magento\Framework\Event\Observer;
use \Magento\Framework\Event\ObserverInterface;

class EmailSetVarsBeforeObserver implements ObserverInterface
{
    /**
     * @var \Magentoj\JapaneseConsumptionTax\Model\Config\JctSystemConfig
     */
    private $jctConfig;

    /**
     * EmailSetVarsBeforeObserver constructor.
     * @param \Magentoj\JapaneseConsumptionTax\Model\Config\JctSystemConfig $jctConfig
     */
    public function __construct(
        \Magentoj\JapaneseConsumptionTax\Model\Config\JctSystemConfig $jctConfig,
    ) {
        $this->jctConfig = $jctConfig;
    }

    public function execute(Observer $observer)
    {
        // Accessing with index because getTransportObject() returns null
        $transportObject = $observer->getEvent()['transportObject'];
        $storeId = $transportObject->getStore()->getStoreId();

        $transportObject->setData('magentoj_jct_registration_number', $this->jctConfig->getRegistrationNumber($storeId));
    }
}
